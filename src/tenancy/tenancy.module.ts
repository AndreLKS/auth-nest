...
const connectionFactory = {
  ...
  useFactory: (request: ExpressRequest) => {
    ...
    const tenantName: string = request.params['tenant'];

    if (tenantName) {
        return getTenantConnection(tenantName, 'clock'); //TODO: set as env var
    } 

    throw new BadRequestException(
      'Database Connection Error',
      'There is a Error with the Database! - Tennant Name is Necessary!',
    );
  },
  inject: [REQUEST],
};
